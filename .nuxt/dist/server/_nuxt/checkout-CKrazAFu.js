import { a as useUserStore, b as __nuxt_component_0 } from "../server.mjs";
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(isLoadingCart) && !unref(cartItems).length) {
        ...
        if (unref(userStore).isLoggedIn) {
          ...
          if (unref(addressStore).isLoading) {
            ...
            ssrRenderList(unref(addressStore).addresses, (address) => {
              ...
              if (unref(selectedAddressId) === address.id) {
      ...
      if (unref(cartItems).length > 0) {
        ...
        ssrRenderList(unref(paymentMethods), (method) => {
          _push(`<button class="${ssrRenderClass([
            ...
            unref(selectedPaymentMethod) === method.id ? "border-cyan-500 bg-cyan-500/20 text-cyan-300" : "border-gray-600/50 hover:border-cyan-500/50 text-gray-300 hover:text-white"
        ...
        if (unref(selectedPaymentMethod) === "wechat") {
      ...
      _push(`</div><div class="lg:col-span-1"><div class="glass-card-dark rounded-2xl border border-cyan-500/30 shadow-2xl shadow-cyan-500/20 sticky top-6"><div class="px-8 py-6 border-b border-gray-700/50"><h2 class="text-2xl font-bold text-white flex items-center gap-3"><i class="bi bi-receipt text-cyan-400"></i> 订单摘要 </h2></div><div class="p-8 space-y-6"><div class="space-y-4"><div class="flex justify-between items-center"><span class="text-gray-300">商品小计</span><span class="text-white font-semibold">¥${ssrInterpolate(unref(subtotal).toLocaleString())}</span></div><div class="flex justify-between items-center"><span class="text-gray-300">运费</span><span class="text-white font-semibold">¥${ssrInterpolate(unref(shipping).toLocaleString())}</span></div><div class="flex justify-between items-center"><span class="text-gray-300">税费</span><span class="text-white font-semibold">¥${ssrInterpolate(unref(tax).toLocaleString())}</span></div><div class="border-t border-gray-700/50 pt-4"><div class="flex justify-between items-center"><span class="text-xl font-semibold text-white">总计</span><span class="text-2xl font-bold text-cyan-400">¥${ssrInterpolate(unref(total).toLocaleString())}</span></div></div></div>`);
      if (unref(cartItems).length > 0) {
        _push(`<div><label class="block text-sm font-medium text-white mb-2">优惠券代码</label><div class="flex gap-2"><input${ssrRenderAttr("value", unref(couponCode))} type="text" placeholder="输入优惠券代码" class="flex-1 px-4 py-3 bg-gray-800/50 border border-gray-600/50 rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 text-white placeholder-gray-400 text-sm"><button class="px-4 py-3 bg-green-600 hover:bg-green-500 text-white font-medium rounded-lg transition-colors duration-300 text-sm"> 使用 </button></div>`);
        if (unref(appliedCoupon)) {
          _push(`<div class="mt-2 text-green-400 text-sm flex items-center gap-1"><i class="bi bi-check-circle"></i> 优惠券&quot;${ssrInterpolate(unref(appliedCoupon))}&quot;已使用 </div>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</div>`);
      ...
      if (unref(cartItems).length > 0) {
        ...
        _push(` ${ssrInterpolate(unref(isProcessing) ? "处理中..." : "确认下单")}</button>`);
      } else {
        _push(`<!---->`);
      }
      if (!unref(canSubmitOrder) && unref(cartItems).length > 0) {
...
//# sourceMappingURL=checkout-CKrazAFu.js.map
